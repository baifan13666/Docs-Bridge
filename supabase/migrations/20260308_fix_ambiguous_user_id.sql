-- Fix ambiguous user_id column reference in search functions
-- Change parameter name from user_id to p_user_id to avoid conflict

-- Drop existing functions
DROP FUNCTION IF EXISTS search_similar_chunks(vector, float, int, uuid, uuid[]);
DROP FUNCTION IF EXISTS search_similar_chunks_coarse(vector, float, int, uuid, uuid[]);

-- Recreate search_similar_chunks with fixed parameter name
CREATE OR REPLACE FUNCTION search_similar_chunks(
  query_embedding vector(384),
  match_threshold float DEFAULT 0.5,
  match_count int DEFAULT 20,
  p_user_id uuid DEFAULT NULL,
  active_folder_ids uuid[] DEFAULT NULL
)
RETURNS TABLE (
  chunk_id uuid,
  document_id uuid,
  chunk_text text,
  similarity float,
  title text,
  source_url text,
  document_type text,
  chunk_index int
)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT
    dc.id AS chunk_id,
    dc.document_id,
    dc.chunk_text,
    1 - (dc.query_embedding <=> query_embedding) AS similarity,
    d.title,
    d.source_url,
    d.document_type,
    dc.chunk_index
  FROM document_chunks dc
  JOIN kb_documents d ON dc.document_id = d.id
  WHERE
    -- Access control: user's own documents OR government documents
    (d.user_id = p_user_id OR d.document_type = 'gov_crawled')
    -- Filter by active folders if provided
    AND (
      active_folder_ids IS NULL 
      OR d.folder_id = ANY(active_folder_ids)
    )
    -- Similarity threshold
    AND 1 - (dc.query_embedding <=> query_embedding) > match_threshold
  ORDER BY dc.query_embedding <=> query_embedding
  LIMIT match_count;
END;
$$;

-- Recreate search_similar_chunks_coarse with fixed parameter name
CREATE OR REPLACE FUNCTION search_similar_chunks_coarse(
  query_embedding vector(384),
  match_threshold float DEFAULT 0.5,
  match_count int DEFAULT 30,
  p_user_id uuid DEFAULT NULL,
  active_folder_ids uuid[] DEFAULT NULL
)
RETURNS TABLE (
  chunk_id uuid,
  document_id uuid,
  chunk_text text,
  similarity float,
  title text,
  source_url text,
  document_type text,
  chunk_index int,
  embedding_large vector(1024)
)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT
    dc.id AS chunk_id,
    dc.document_id,
    dc.chunk_text,
    1 - (dc.query_embedding <=> query_embedding) AS similarity,
    d.title,
    d.source_url,
    d.document_type,
    dc.chunk_index,
    dc.embedding_large
  FROM document_chunks dc
  JOIN kb_documents d ON dc.document_id = d.id
  WHERE
    -- Access control: user's own documents OR government documents
    (d.user_id = p_user_id OR d.document_type = 'gov_crawled')
    -- Filter by active folders if provided
    AND (
      active_folder_ids IS NULL 
      OR d.folder_id = ANY(active_folder_ids)
    )
    -- Similarity threshold
    AND 1 - (dc.query_embedding <=> query_embedding) > match_threshold
  ORDER BY dc.query_embedding <=> query_embedding
  LIMIT match_count;
END;
$$;

-- Add comments
COMMENT ON FUNCTION search_similar_chunks IS 
'Performs semantic search on document chunks using cosine similarity. 
Filters by user access and active folders. 
Uses 384-dim embeddings from browser (e5-small).
Fixed: Renamed user_id parameter to p_user_id to avoid ambiguity.';

COMMENT ON FUNCTION search_similar_chunks_coarse IS 
'Performs coarse semantic search returning 1024-dim embeddings for reranking. 
Filters by user access and active folders. 
Uses 384-dim embeddings for initial search.
Fixed: Renamed user_id parameter to p_user_id to avoid ambiguity.';
